...
- **High-level product requirements** (domain concepts + UI areas): **Obsidian** ProjectTracker → `Projects/Destiny 2 Build Creator/` (working copy). Local soft link: [`requirements/`](./requirements/) (recreate with `scripts/link-projecttracker-requirements.ps1`). Git pointer: [`docs/products/README.md`](./docs/products/README.md).
...
- High-level product requirements: **Obsidian** `Projects/Destiny 2 Build Creator/` via local `requirements/` mount (Domains + Areas + Destiny Objects); pointer `docs/products/README.md`.
