...
/// Host orchestration for DIM jsonOnly clipboard export (DART-039).
///
/// Enforces equip-ready hard gate via [buildJsonOnlyDimExport]. Soft guidance
/// never applies. No network / dim.gg / CLIENT_SECRET.
class DimExportController extends ChangeNotifier {
  ...
  /// Optional fixed loadout id for tests; production generates a unique id.
  ...
  /// Bind to current build/variant selection and refresh readiness.
  Future<void> bind({
    required int userId,
  ...
  /// Request jsonOnly clipboard export.
  ///
  /// Returns null on success; error string when blocked or failed.
  /// Hard gate: [buildJsonOnlyDimExport] asserts equip-ready — no clipboard
  /// write when not ready.
  Future<String?> requestExport() async {
    ...
    try {
      // Re-evaluate readiness before any clipboard write.
      await refreshReadiness();
      ...
      final payload = buildJsonOnlyDimExport(
        ...
        input: VariantDimLoadoutInput(
          ...
          softStatTargets:
              _softStatTargets.isEmpty ? null : _softStatTargets,
