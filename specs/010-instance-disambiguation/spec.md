...
**Out of scope**: Attaching to a **build or build variant** (sets only this iteration; builds reuse this picker in a follow-on when a build item attachment surface exists); a **mixed carousel of different items** (the carousel shows copies of one selected item only); production-polished picker UI; semantic/AI search; keyword description search (delivered by [009-description-search](../009-description-search/spec.md)); changing inventory sync or manifest ingestion; **capping or paginating** the candidate list (all owned copies are shown); virtualization of very large candidate lists; multi-select attachment (attaching more than one instance at once); persisting a user's removed-candidate list across sessions; roll-hunting suggestions.
