...
| DBR-IDL-005 | **Preferred score N/M** (roll quality): among plugs **on this copy** in sockets that have preferred and/or avoid multi-picks (equipped + reusables), N = plugs that match any preferred **manifest hash**, M = count of those plugs. Example: 6 perks on Trait 1+2 with 3 ideal → `3/6`. **Perfect / success tint** = every preferred multi-pick **column** has a hit (multi-accept), not N==M. |
| DBR-IDL-006 | **Avoid score Av k**: among the same on-copy plugs, k = plugs that match any avoid **manifest hash**. Higher hits = worse. |
