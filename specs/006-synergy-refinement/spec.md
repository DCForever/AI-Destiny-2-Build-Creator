...
**In scope**: Refining how synergies are defined and verified in internal debug tools—auto-generated names, specific sub-type selection for Verb/Melee/Grenade synergies, catalog-backed link pickers (no free-text entry), and read-only description display for linked game elements.

**Out of scope**: Polished production synergy editor UI, changes to suggestion-ranking algorithms, and bulk migration of legacy synergies (existing records remain readable; new/edited synergies follow the refined rules).
...
A curator creating a synergy should not need to type a display name manually. As they choose the synergy category, any required sub-type (e.g., a specific verb), and a linked game element, the system composes a human-readable name that reflects those choices—such as **Verb: Scorch — Skyburner's Oath**.
...
### User Story 2 - Specific Sub-Types for Verb, Melee, and Grenade (Priority: P1)

Verb, Melee, and Grenade synergies must identify a **specific** gameplay element—not merely the broad category. For verbs, the user picks a named verb (e.g., Scorch, Sever). For grenades, the user picks a specific grenade type. For melee, the user picks a specific melee ability. Other synergy categories (e.g., Super, Damage, Healing, weapon-slot types) continue to use category alone without an additional sub-type picker.

**Why this priority**: A generic "Verb" synergy is too vague to guide builds or suggestions; curators need precision about which verb, grenade, or melee the synergy documents.

**Independent Test**: Attempt to create a Verb synergy without selecting a specific verb; save is blocked. Create Verb + Scorch + one link; synergy is saved with type metadata reflecting Scorch.
...
2. **Given** the user selects category Grenade, **When** the form is shown, **Then** a required sub-type picker lists specific grenade types from curated game data and does not allow saving without a selection.
3. **Given** the user selects category Melee, **When** the form is shown, **Then** a required sub-type picker lists specific melee abilities from curated game data and does not allow saving without a selection.
4. **Given** the user selects a category that does not require a sub-type (e.g., Super), **When** they complete the form, **Then** no sub-type picker is shown and naming uses category + link only (e.g., `Super — Hammer of Sol`).
5. **Given** an existing synergy created under the old generic Verb model, **When** the user views it, **Then** it remains visible and editable; saving after edit must satisfy the new sub-type requirement.
...
- Sub-type vocabularies (verbs, grenades, melees) should be deduplicated across subclasses where the same name appears multiple times (e.g., Scorch on multiple subclasses → one Scorch option).

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: System MUST auto-generate synergy display names from the selected category, required sub-type (when applicable), and primary linked element display name, using a consistent pattern: `{Category}: {Sub-type} — {Link name}` for Verb/Melee/Grenade, and `{Category} — {Link name}` for categories without sub-types.
...
- **FR-003**: System MUST require a specific grenade selection when synergy category is Grenade; generic Grenade-only synergies MUST NOT be creatable or savable without a named grenade sub-type.
- **FR-004**: System MUST require a specific melee ability selection when synergy category is Melee; generic Melee-only synergies MUST NOT be creatable or savable without a named melee sub-type.
- **FR-005**: Sub-type options for Verb, Melee, and Grenade MUST be drawn from curated, system-maintained vocabularies aligned with Destiny 2 gameplay (not user-typed strings).
- **FR-006**: Synergy link pickers for weapon, weapon_perk, origin_trait, and armor_set_bonus MUST be populated exclusively from catalog data; free-text or raw numeric hash entry for these link kinds MUST NOT be available in verification UI.
- **FR-007**: Selecting a link from catalog pickers MUST populate all required link fields (identity, display name, and kind-specific attributes) needed for validation and persistence per existing synergy link rules.
- **FR-008**: When a weapon, weapon_perk, origin_trait, or armor_set_bonus is selected as a link, the verification UI MUST display that element's catalog description read-only before save.
- **FR-009**: Auto-generated names MUST update live in the create/edit form when category, sub-type, or link selection changes, prior to submit.
- **FR-010**: Validation MUST reject synergy create/update when any required sub-type is missing or when any link cannot be resolved against current catalog data, with a clear error message.
- **FR-011**: Existing synergies created before this refinement MUST remain listable and deletable; editing and re-saving MUST conform to the refined rules.

### Key Entities

- **Synergy**: A documented interaction with category, optional sub-type (for Verb/Melee/Grenade), auto-generated name, optional curator notes, and one or more links to game elements.
- **Synergy sub-type**: A specific verb, grenade type, or melee ability that narrows a Verb/Melee/Grenade synergy beyond its broad category.
...
- **SC-004**: Auto-generated names match the defined pattern for at least 95% of tested combinations across Verb, Melee, Grenade, and non-sub-type categories in acceptance testing.
...
- Grenade and melee sub-type lists are derived from curated subclass/ability metadata or equivalent catalog sources maintained by the project.
