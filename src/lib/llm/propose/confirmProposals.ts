...
function unknownPassError(passId: string): ApiError {
  return new ApiError(
...
export async function confirmProposals(
  ...
  // Only server-held proposals for this pass may be confirmed (no client replay).
  const pass = getProposePass(passId);
  if (!pass) {
    throw unknownPassError(passId);
  }
  if (pass.userId !== undefined && pass.userId !== userId) {
    throw unknownPassError(passId);
  }
