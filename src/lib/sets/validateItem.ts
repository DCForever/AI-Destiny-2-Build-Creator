...
export async function validateItemHash(itemHash: number): Promise<ItemValidationResult> {
  ...
  const stores = ["weapons", "exotic-weapons", "exotic-armor"] as const;

  for (const store of stores) {
    const records = await entityCache.getStore(store);
    const match = records.find((r) => r.hash === itemHash);
    if (match) {
      return { valid: true, name: match.name, stale: false };
    }
  }
...
export async function resolveItemDisplayName(
  itemHash: number,
  fallbackName?: string,
): Promise<{ name: string; stale: boolean }> {
  const result = await validateItemHash(itemHash);
  if (result.valid && result.name) {
    return { name: result.name, stale: false };
  }
  return { name: fallbackName ?? `Unknown (${itemHash})`, stale: true };
}
