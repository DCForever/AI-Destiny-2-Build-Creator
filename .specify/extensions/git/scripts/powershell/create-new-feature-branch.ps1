...
function Switch-ToConfiguredBaseBranch {
    param(
        [string]$RepoRoot,
        [string]$TargetBranchName
    )

    $baseBranch = Get-ConfiguredBaseBranch -RepoRoot $RepoRoot
    if (-not $baseBranch) { return $baseBranch }

    $current = ''
    try { $current = (git rev-parse --abbrev-ref HEAD 2>$null).Trim() } catch {}
    if ($current -eq $baseBranch) { return $baseBranch }

    git show-ref --verify --quiet "refs/heads/$baseBranch" 2>$null
    if ($LASTEXITCODE -eq 0) {
        $checkoutError = git checkout -q $baseBranch 2>&1 | Out-String
        if ($LASTEXITCODE -ne 0) {
            Write-Error "Error: Could not check out base branch '$baseBranch' before creating '$TargetBranchName'.`n$($checkoutError.Trim())"
            exit 1
        }
        return $baseBranch
    }
    ...
    if ($LASTEXITCODE -eq 0) {
        $checkoutError = git checkout -q -B $baseBranch "origin/$baseBranch" 2>&1 | Out-String
        if ($LASTEXITCODE -ne 0) {
            Write-Error "Error: Could not check out origin/$baseBranch before creating '$TargetBranchName'.`n$($checkoutError.Trim())"
            exit 1
        }
        return $baseBranch
    }

    Write-Error "Error: base_branch '$baseBranch' is configured in git-config.yml but was not found locally or as origin/$baseBranch."
    exit 1
}
...
$configuredBaseBranch = Get-ConfiguredBaseBranch -RepoRoot $repoRoot

if (-not $DryRun) {
    if ($hasGit) {
        $existingBranch = git branch --list $branchName 2>$null
        $skipBaseCheckout = $existingBranch -and $AllowExistingBranch

        if (-not $skipBaseCheckout) {
            Switch-ToConfiguredBaseBranch -RepoRoot $repoRoot -TargetBranchName $branchName | Out-Null
        }
...
if ($Json) {
    ...
    if ($configuredBaseBranch) {
        $obj | Add-Member -NotePropertyName 'BASE_BRANCH' -NotePropertyValue $configuredBaseBranch
    }
    if ($DryRun) {
