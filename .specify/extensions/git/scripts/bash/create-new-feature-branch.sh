...
switch_to_configured_base_branch() {
    local target_branch="$1"
    local base_branch current_branch checkout_error
    ...
    if git show-ref --verify --quiet "refs/heads/$base_branch" 2>/dev/null; then
        if ! checkout_error=$(git checkout -q "$base_branch" 2>&1); then
            >&2 echo "Error: Could not check out base branch '$base_branch' before creating '$target_branch'."
            [ -n "$checkout_error" ] && >&2 printf '%s\n' "$checkout_error"
            exit 1
        fi
        return 0
    fi
    ...
    if git show-ref --verify --quiet "refs/remotes/origin/$base_branch" 2>/dev/null; then
        if ! checkout_error=$(git checkout -q -B "$base_branch" "origin/$base_branch" 2>&1); then
            >&2 echo "Error: Could not check out origin/$base_branch before creating '$target_branch'."
            [ -n "$checkout_error" ] && >&2 printf '%s\n' "$checkout_error"
            exit 1
        fi
        return 0
    ...
    >&2 echo "Error: base_branch '$base_branch' is configured in git-config.yml but was not found locally or as origin/$base_branch."
    exit 1
}

CONFIGURED_BASE_BRANCH="$(get_configured_base_branch)"
...
if [ "$DRY_RUN" != true ]; then
    if [ "$HAS_GIT" = true ]; then
        skip_base_checkout=false
        if git branch --list "$BRANCH_NAME" | grep -q . && [ "$ALLOW_EXISTING" = true ]; then
            skip_base_checkout=true
        fi
        if [ "$skip_base_checkout" != true ]; then
            switch_to_configured_base_branch "$BRANCH_NAME"
        fi
...
if $JSON_MODE; then
    if command -v jq >/dev/null 2>&1; then
        if [ "$DRY_RUN" = true ]; then
            if [ -n "$CONFIGURED_BASE_BRANCH" ]; then
                jq -cn \
                    ...
                    --arg base_branch "$CONFIGURED_BASE_BRANCH" \
                    '{BRANCH_NAME:$branch_name,FEATURE_NUM:$feature_num,BASE_BRANCH:$base_branch,DRY_RUN:true}'
            else
                jq -cn \
                    --arg branch_name "$BRANCH_NAME" \
                    --arg feature_num "$FEATURE_NUM" \
        ...
        elif [ -n "$CONFIGURED_BASE_BRANCH" ]; then
            jq -cn \
                --arg branch_name "$BRANCH_NAME" \
                --arg feature_num "$FEATURE_NUM" \
                --arg base_branch "$CONFIGURED_BASE_BRANCH" \
                '{BRANCH_NAME:$branch_name,FEATURE_NUM:$feature_num,BASE_BRANCH:$base_branch}'
        else
            jq -cn \
                --arg branch_name "$BRANCH_NAME" \
                --arg feature_num "$FEATURE_NUM" \
