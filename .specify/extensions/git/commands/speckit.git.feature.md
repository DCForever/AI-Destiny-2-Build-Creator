...
Before creating a new feature branch, check `.specify/extensions/git/git-config.yml` for `base_branch`:

1. If set (e.g. `feature/overhall`), the script checks out that branch first (local branch, then `origin/<name>`)
2. If unset, the new branch is created from the current `HEAD`
3. When switching to an existing branch with `--allow-existing-branch`, base checkout is skipped
...
- `BASE_BRANCH`: Present when `base_branch` is configured in `git-config.yml`
