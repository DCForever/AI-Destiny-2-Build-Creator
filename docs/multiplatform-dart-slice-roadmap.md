...
| **DART-072** | **done** | `catalog-nested-group-tree` | `dart-072-catalog-nested-group-tree` | P10b | DART-062 flat group-by pure API | Pure **nested group tree** + rollup counts from ordered dims | Closes **system half** of **GAP-UI-CATALOG-11** / **FEAT-UI-CATALOG-NESTED-GROUP** (`e711190`). Pure tests; **no** nested header chrome / JUMP UI. Chrome → **UX-CATALOG-NESTED-GROUP** (005 brief locked). BR-CAT-006 semantics unchanged. **Not cutover re-gate.** Soft never auto-applies; no CLIENT_SECRET |
