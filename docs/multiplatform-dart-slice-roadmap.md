...
**Updated:** 2026-07-24 (DART-002 done)  
...
| Field | Value |
| ----- | ----- |
| **Next / active slice** | **DART-003** `hard-constraints` (pure hard evaluators — depends on DART-002 done) |
| **Active branch** | (create) `dart-003-hard-constraints` from `feature/multiplatform-dart` |
| **Specs dir** | `specs/dart-003-hard-constraints/` (created at specify) |
| **Active worktree** | `F:\Destiny2BuildCreator-multiplatform-dart` |
| **Blocked on** | — |

### DART-002 scope boundary (important)

**DART-002 is pure models only.**  
Spec Kit specify must **not** include hard/soft evaluator algorithms — those are **DART-003+**. Models live in `packages/domain` with zero IO.
