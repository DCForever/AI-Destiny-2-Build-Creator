...
/// Host orchestration for equip UI (DART-038).
///
/// Enforces equip-ready hard gate, class match, optional gaps confirm for empty
/// combat slots, then plan + best-effort execute. Soft guidance never applies.
class EquipController extends ChangeNotifier {
  ...
  ///
  /// When [forceGapsConfirm] is false (default) and empty combat slots exist,
  /// sets [pendingGaps] and returns null without writing.
  Future<String?> requestEquip({bool forceGapsConfirm = false}) async {
    ...
    // Re-evaluate readiness before any write.
  ...
  Future<String?> _executeEquip(String characterId) async {
    ...
    try {
      ...
      // Authoritative post-sync readiness (product equip route).
      final resolved =
      ...
      EquipPlanArtifact? artifact;
      // Variant artifact is not always on resolved equipment; skip if unknown.
